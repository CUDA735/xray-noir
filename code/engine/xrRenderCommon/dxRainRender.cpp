...
constexpr float drop_angle = 3.0f;
constexpr float drop_max_angle = deg2rad(10.f);
...
void dxRainRender::Copy(IRainRender& _in) { 
    *this = static_cast<const dxRainRender&>(_in); 
}
...
void dxRainRender::Render(CEffect_Rain& owner) {
    ...
    for (auto& one : owner.items) {
        ...
        Fvector wdir;
        wdir.set(one.P.x - vEye.x, 0.f, one.P.z - vEye.z);
        ...
        verts->set(P, u_rain_color, UV[s][0].x, UV[s][0].y); verts++;
        
        P.mad(pos_trail, lineTop, w);
        verts->set(P, u_rain_color, UV[s][1].x, UV[s][1].y); verts++;
        
        P.mad(pos_head, lineTop, -w);
        verts->set(P, u_rain_color, UV[s][2].x, UV[s][2].y); verts++;
        
        P.mad(pos_head, lineTop, w);
        verts->set(P, u_rain_color, UV[s][3].x, UV[s][3].y); verts++;
    }
